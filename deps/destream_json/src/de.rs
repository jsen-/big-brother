...
/// Decode the given JSON-encoded stream of bytes into a stream of instances of `T` using the given context.
pub async fn try_decode_iter<
    E: fmt::Display,
    S: Stream<Item = Result<Bytes, E>> + Send + Unpin,
    T: FromStream,
>(
    ...
    T::Context: Clone,
{
    async_stream::stream! {
        let mut decoder = Decoder::from_stream(source.map_err(|e| de::Error::custom(e)));
        loop {
            yield T::from_stream(context.clone(), &mut decoder).await;
            if let Err(err) = decoder.expect_whitespace().await {
                yield Err(err);
            }
            if decoder.is_terminated() {
                return;
